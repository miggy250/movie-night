...
import { Clock, ArrowLeft, Search, Filter, Grid, List, Play } from 'lucide-react';
...
export default function WatchLaterPage() {
  ...
  const filteredWatchLater = watchLater.filter(movie =>
    movie.title.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const sortedWatchLater = [...filteredWatchLater].sort((a, b) => {
    switch (sortBy) {
      case 'title':
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-3">
              <Clock className="w-6 h-6 text-green-500 fill-green-500" />
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div className="text-gray-400">
              {watchLater.length === 0 ? (
                <span>No movies saved for later</span>
              ) : (
                <span>{watchLater.length} {watchLater.length === 1 ? 'movie' : 'movies'} to watch</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-10 pr-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-green-600 transition-colors"
                />
              </div>
              ...
              <select
                ...
                className="px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-green-600 transition-colors"
              >
        ...
        <div className="px-4 sm:px-6 lg:px-8">
          {watchLater.length === 0 ? (
            <div className="text-center py-20">
              ...
              <h2 className="text-2xl font-bold text-white mb-4">No movies saved yet</h2>
              <p className="text-gray-400 mb-8 max-w-md mx-auto">
                Build your watch list by clicking the clock icon on any movie card or in the movie details.
              </p>
              <button
                onClick={() => window.history.back()}
            ...
            >
              {sortedWatchLater.map((movie, index) => (
                <motion.div
                  key={movie.id}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
